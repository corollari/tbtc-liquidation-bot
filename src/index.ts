...
bot.onText(/\/watch (.+)/, async (msg, match) => {
  ...
  if(match===null){
    bot.sendMessage(chatId, "You must provide the ethereum address of the deposit to watch");
    return;
  }
  const address = match[1];
  const ethereumAddressRegex = /^(0x)[0-9A-Fa-f]{40}$/
  if(!ethereumAddressRegex.test(address)){
    bot.sendMessage(chatId, "The address provided is not a valid ethereum address, please try again with a different address");
    return;
  }
  try{
    await (await tbtc).Deposit.withAddress(address)
  } catch(e){
    bot.sendMessage(chatId, "The address provided does not correspond to a tbtc deposit, please try again with a different address");
    return
  }
  ...
  redisClient.sadd(address, String(chatId), (err, res)=>{
    ...
    if(res===0){
      ...
      bot.sendMessage(chatId, `Your deposit has been registered, we will send you an update if it ever fails below the first threshold (where it could get courtesy-called).`);
    }
  })
});
...
bot.onText(/\/watch (.+)/, (msg, match) => {
  const chatId = msg.chat.id;
  if(match===null){
    bot.sendMessage(chatId, "You must provide the ethereum address of the deposit to unwatch");
    return;
  }
  const address = match[1];
  ...
  redisClient.srem(address, String(chatId), (_, res)=>{
    if(res === 0){
      bot.sendMessage(chatId, "You were not subscribed to this deposit so no action has been taken")
    } else {
      bot.sendMessage(chatId, "You have been successfuly unsubscribed from alerts on this deposit")
    }
  })
});
